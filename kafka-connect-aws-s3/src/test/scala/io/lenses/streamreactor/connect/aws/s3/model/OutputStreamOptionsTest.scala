...
class OutputStreamOptionsTest extends AnyFlatSpec with Matchers {
  ...
  it should "create BuildLocalOutputStreamOptions when temp directory has been supplied" in {
    OutputStreamOptions(adapt(Map(LOCAL_TMP_DIRECTORY -> s"$tmpDir/my/path"))) should
      be(Right(OutputStreamOptions(LocalRootLocation(s"$tmpDir/my/path"))))
  }
  ...
  it should "create BuildLocalOutputStreamOptions when temp directory and sink name has been supplied" in {
    ...
    OutputStreamOptions(adapt(Map(LOCAL_TMP_DIRECTORY -> s"$tmpDir/my/path", "name" -> "superSleekSinkName"))) should
      be(Right(OutputStreamOptions(LocalRootLocation(s"$tmpDir/my/path"))))
  }
  ...
  it should "not create BuildLocalOutputStreamOptions when nothing supplied" in {
    val result = OutputStreamOptions(adapt(Map[String, String]()))
